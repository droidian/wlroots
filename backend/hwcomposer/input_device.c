...
bool wlr_input_device_is_hwcomposer(struct wlr_input_device *wlr_dev) {
...
struct wlr_input_device *wlr_hwcomposer_add_input_device(
	...
	struct wlr_hwcomposer_input_device *device =
		calloc(1, sizeof(struct wlr_hwcomposer_input_device));
	if (device == NULL) {
		return NULL;
	}
